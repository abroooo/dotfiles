...
function ShowTODOsInCommit(commit)
  local cmd = 'git diff ' .. commit .. ' | grep TODO'
  -- builtin.live_grep.grep_string {
...
local previewers = require 'telescope.previewers'
function ShowBla()
  local cmd = { 'sh', '-c', 'git diff HEAD..HEAD~8 | grep TODO' }
  -- local cmd = 'git diff HEAD..HEAD~8 | grep TODO'
  require('telescope.pickers')
    .new({}, {
      finder = require('telescope.finders').new_oneshot_job(cmd),
      sorter = require('telescope.sorters').get_generic_fuzzy_sorter(),
...
vim.api.nvim_set_keymap('n', '<leader>td', ":lua ShowTODOsInCommit('<commit_hash>')<CR>", { noremap = true, silent = true })
local telescope = require 'telescope.builtin'

function ShowTODOsInSelectedCommit()
  telescope.git_commits {
    attach_mappings = function(_, map)
      map('i', '<CR>', function(prompt_bufnr)
        local entry = require('telescope.actions.state').get_selected_entry(prompt_bufnr)
        local commit_hash = entry.value -- Get selected commit hash
        require('telescope.actions').close(prompt_bufnr)
        print(commit_hash)

        -- Run grep_string or custom command for TODOs
        local cmd = 'git diff ' .. commit_hash .. ' | grep TODO'
        vim.fn.system(cmd)
        telescope.live_grep {
          search = 'TODO',
          prompt_title = 'TODOs in Commit ' .. commit_hash,
        }
      end)
      return true
    end,
...
--     "n",
--     "<leader>tc",
--     ":lua ShowTODOsInSelectedCommit()<CR>",
